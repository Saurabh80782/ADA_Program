...
void multiplyStrassen(int n, int A[n][n], int B[n][n], int C[n][n]) {
    ...
    add(mid, A11, A22, T1);
    add(mid, B11, B22, T2);
    multiplyStrassen(mid, T1, T2, M1);

    add(mid, A21, A22, T1);
    multiplyStrassen(mid, T1, B11, M2);

    sub(mid, B12, B22, T2);
    multiplyStrassen(mid, A11, T2, M3);

    sub(mid, B21, B11, T2);
    multiplyStrassen(mid, A22, T2, M4);

    add(mid, A11, A12, T1);
    multiplyStrassen(mid, T1, B22, M5);

    sub(mid, A21, A11, T1);
    add(mid, B11, B12, T2);
    multiplyStrassen(mid, T1, T2, M6);

    sub(mid, A12, A22, T1);
    add(mid, B21, B22, T2);
    multiplyStrassen(mid, T1, T2, M7);
